...
| gmail | `gmail` | 25 pass | Yes | Send/read, reply, drafts, labels, mark read/unread, trash - **Refactored** |
...
| Tool | Module | Tests | Live Tested | Notes |
|------|--------|-------|-------------|-------|
| google_auth | `google_auth` | 7 pass | Yes | OAuth authentication helper |
...
| google_sheets | `google_sheets` | 15 pass | Yes | **Refactored** - explicit params |
...
| Tool | Module | Tests | Live Tested | Notes |
|------|--------|-------|-------------|-------|
| youtube_read | `youtube` | 19 pass | Yes | **Refactored** - Read-only (API key only). `youtube` is an alias. 6 actions: search, get_videos, get_channels, list_playlists, list_playlist_items, get_comments. Uses YOUTUBE_CHANNEL_ID env default. |
| youtube_write | `youtube_write` | 9 pass | Yes | **Refactored** - 10 actions: update_video_metadata, playlist CRUD, delete_video, set_thumbnail, set_video_privacy |
...
| sns | `sns` | 14 pass | Yes | 10 actions: create/delete/list_topics, get_topic_attributes, publish, subscribe, subscribe_lambda, unsubscribe, list_subscriptions, confirm_subscription - **Refactored** |
| sqs | `sqs` | 13 pass | Yes | 12 actions: create/delete/list_queues, get_queue_url, get_queue_attributes, send, send_batch, receive, delete_message, delete_message_batch, purge, change_visibility - **Refactored** |
...
| Tool | Module | Tests | Live Tested | Notes |
|------|--------|-------|-------------|-------|
| sqlite | `sqlite` | 23 pass | Yes | 17 actions: execute, query, create/drop table, insert, update, delete, upsert, list/describe tables, get_info, backup, vacuum, create/drop index, truncate, export/import CSV - **Refactored** |
...
| chromadb | `chromadb_tool` | 21 pass | Yes | 14 actions: create/get/get_or_create/list/delete/modify collection, add, query, get, update, upsert, delete, count, peek - **Refactored** |
| keyword_search | `keyword_search` | 13 pass | Yes | 5 actions: create_index, search, add_documents, delete_index, list_indexes - BM25 keyword search for hybrid retrieval - **Refactored** |
...
| ffmpeg | `ffmpeg` | 42 pass | Yes | 14 actions: cut, concat, info, extract_audio, resize, convert, compress, add_audio, extract_frames, create_gif, thumbnail, rotate, speed, watermark - **Refactored** |
...
| pdf_to_markdown | `pdf_to_markdown` | 10 pass | Yes | PDF to markdown conversion - local files and arXiv papers via ar5iv - **Refactored** |

## AI & Generation

| Tool | Module | Tests | Live Tested | Notes |
|------|--------|-------|-------------|-------|
| gemini_image | `gemini_image` | 28 pass | Yes | **Refactored** - 2 actions: generate, edit - output_format, output_filename, num_images support |
| gemini_video | `gemini_video` | 36 pass | Yes | **Refactored** - Veo 3.1 - text/image to video, 1080p, talking/dialogue, commercials, action, horror |
| gemini_music | `gemini_music` | 17 pass | Yes | **Refactored** - Lyria - BPM/density/brightness controls, orchestral, lo-fi, ⚠️ no artist names |
